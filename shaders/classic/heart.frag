...
void main()
{
	...
	vec3 diffuseColor = texture(Diffuse, In.Texcoord  * Time).rgb;
	vec3 specularColor = texture(Specular, In.Texcoord  * Time).rgb;

    //vec3 I = normalize(PositionR - In.CameraSpacePosition);
    //vec3 R = reflect(I, normalize(NormalR));
    vec3 reflectColor = texture(ReflectMap, In.Texcoord).rgb;
	...
	//FragColor = vec4(vec3(color.x), 1.0);
