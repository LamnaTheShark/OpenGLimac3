...
{	
	...
	float ct = cos(t);
	float st = sin(t);
	p.x = Position.x * ct + Position.y * st;
	p.y = -Position.x * st + Position.y * ct;
	n.x = Normal.x * ct + Normal.y * st;
	n.y = -Normal.x * st + Normal.y * ct;
	n.z = Normal.z;
	...
	*/
    ...
    //Out.NormalR = mat3(transpose(inverse(MVP))) * Normal;
    //Out.PositionR = vec3(MVP * vec4(Position, 1.0f));
